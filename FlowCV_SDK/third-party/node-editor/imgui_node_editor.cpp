...
{
    0.1f, 0.15f, 0.20f, 0.25f, 0.33f, 0.5f, 0.75f, 1.0f, 1.5f
};
...
void ed::NavigateAction::NavigateTo(const ImRect& bounds, bool zoomIn, float duration, NavigationReason reason)
{
    ...
    else
    {
        ...
        auto scrMin = ImMin(ImGui::GetWindowWidth(), ImGui::GetWindowHeight());
        if (scrMin / extend >= s_ZoomLevels[s_ZoomLevelCount - 1]) {
            float scrWRatio = ImGui::GetWindowWidth() / s_ZoomLevels[s_ZoomLevelCount - 1];
            float scrHRatio = ImGui::GetWindowHeight() / s_ZoomLevels[s_ZoomLevelCount - 1];
            float ratioW = scrWRatio / extend;
            float ratioH = scrHRatio / extend;
            auto rSize = rect.GetSize();
            rect.Min.x = targetCenter.x - ((rSize.x * ratioW) / 2.0f);
            rect.Min.y = targetCenter.y - ((rSize.y * ratioH) / 2.0f);
            rect.Max.x = targetCenter.x + ((rSize.x * ratioW) / 2.0f);
            rect.Max.y = targetCenter.y + ((rSize.y * ratioH) / 2.0f);
        }
        else {
